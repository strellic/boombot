...
export default {
  data: new SlashCommandBuilder()
    ...
    .addNumberOption((option) => option.setName('number')
      .setDescription('An optional parameter to choose how many songs to skip')
      .setRequired(false)),
  async execute(client: CustomClient, interaction: CustomCommandInteraction) {
    ...
    if (number && (Number.isNaN(number) || number <= 0)) {
      await interaction.reply({ content: 'The number to skip must be a positive number!', ephemeral: true });
      return;
    }
    ...
    if (subscription) {
      ...
      if (number) {
        ...
        await interaction.reply(`
          Skipped **${(subscription.audioPlayer.state.resource as AudioResource<Track>).metadata.title}**!
        `);
      }
    } else {
