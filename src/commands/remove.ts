...
export default {
  ...
  async execute(client: CustomClient, interaction: CustomCommandInteraction) {
    ...
    const subscription = client.data.subscriptions.get(interaction.guild.id) as MusicSubscription;
    ...
    if (Number.isNaN(number) || number <= 0) {
      await interaction.reply({ content: 'The number to remove must be a positive number!', ephemeral: true });
      return;
    }
    ...
    if (subscription) {
      ...
      if (removed) {
        await interaction.reply(`Removed song **${removed.title}** from the queue!`);
      } else {
        await interaction.reply({ content: 'Could not find song to remove!', ephemeral: true });
      }
    } else {
