...
export default {
  ...
  async execute(client: CustomClient, interaction: CustomCommandInteraction) {
    ...
    if (!time) {
      await interaction.reply({ content: 'Missing time to seek to.', ephemeral: true });
      return;
    }

    const secs = helpers.timestampToSeconds(time);
    if (Number.isNaN(secs)) {
      await interaction.reply({ content: 'Invalid timestamp! Timestamp should be of the form MM:SS.', ephemeral: true });
      return;
    }
    ...
    if (subscription) {
      ...
      await interaction.reply(`Seeking in **${track.title}** to **${time}**...`);
    } else {
