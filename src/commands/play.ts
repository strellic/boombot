...
export default {
  data: new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
      option
        .setName("number")
        .setDescription(
          "An optional number for the position to insert the song at in the queue"
        )
        .setRequired(false)
    ),
  async execute(client: CustomClient, interaction: CustomCommandInteraction) {
    ...
    if (number !== null && (Number.isNaN(number) || number <= 0)) {
      await interaction.reply({
        content: "The insert number must be a positive number.",
        ephemeral: true,
      });
      return;
    }
    ...
    if (playlist && playlistInfo) {
      await interaction.followUp(
        `Added the playlist **${playlist.title}** to the queue!`
      );
      ...
      if (trackInfo) {
        await interaction.followUp(
          `Added **${trackInfo.title}** to the queue!`
        );
      } else {
