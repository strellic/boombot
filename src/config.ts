...
const config: Config = {
  ...
  init: (): boolean => {
    ...
    if (!process.env.CLIENT_ID) {
      log.error(
        'Missing bot client id! Set the "CLIENT_ID" environment variable.'
      );
      return false;
    }
    ...
    if (process.env.DEVELOPMENT && process.env.DEVELOPMENT === "true") {
      if (process.env.DEVELOPMENT_TEST_GUILD) {
        ...
        log.error(
          'Development mode is on, but the test guild id is missing! Set the "DEVELOPMENT_TEST_GUILD" environment variable.'
        );
        return false;
      }
    }
